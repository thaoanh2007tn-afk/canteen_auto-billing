...
Hệ thống tính tiền tự động cho căn tin trường , sử dụng camera và mạng nơ-ron tích chập (CNN) để nhận diện món ăn trên khay cơm và tự động tính hóa đơn — không cần nhân viên nhập liệu thủ công.
...
- Chưa đếm được số lượng trứng thêm trong từng món (bỏ YOLO đếm trứng theo yêu cầu đề bài) 
- Tọa độ 5 vùng khay giả định cố định theo loại khay hiện tại, đổi loại khay cần tinh chỉnh lại qua giao diện.
- Độ chính xác phụ thuộc vào chất lượng,số lượng ảnh huấn luyện; nhóm sẽ tiếp tục bổ sung dữ liệu ở nhiều điều kiện ánh sáng và góc chụp khác nhau.
